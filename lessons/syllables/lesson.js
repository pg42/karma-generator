...
function initialize(karma,content) {
	...
	if(!$('#gameOver').length)
		$('#content').after(createDiv('gameOver').addClass('gameOver').append('Game Over!'));
}
...
function startLesson(karma) {
    ...
    var check_game_over = function() {
		...
		/*else if(totalCounter === 20) {
			...
			alert("player:"+playerCounter +",comp : "+compCounter);
